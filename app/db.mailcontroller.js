...
export async function sendOrderEmail(data) {
  try {
    ...
    let html;
    if (interval == "ONETIME") {
      html = template?.orderTemplate?.html?.replace(
        "{{customerName}}",
        `${data?.customerName}`,
      );
      html = html?.replace(
    ...
    console.log(customerPhone, "data?.customerPhone", data);
...
export async function sendApplyEmail(data) {
  try {
    ...
    const rows = data?.ticketDetails?.appliedForDetail[index]?.appliedList
      .map(
        (id) => `<tr>
    ...
    let html = template?.appliedTemplate?.html?.replace(
      "{{customerName}}",
      `${data?.customerName}`,
    );
    ...
    html = html?.replace(
      "{{productName}}",
      `${data?.ticketDetails?.appliedForDetail[index]?.productName}`,
    );
    html = html?.replace(
      "{{drawIdsLength}}",
      `${data?.ticketDetails?.appliedForDetail[index]?.appliedList?.length}`,
    );
    html = html?.replace("{{drawIdsList}}", `${drawIdsList}`);
    html = html?.replace("{{rows}}", `${rows}`);
...
export async function sendWinnerEmail(data) {
  try {
    ...
    if (template) {
      ...
      let html = template?.winningTemplate?.html?.replace(
        "{{customerName}}",
        `${data?.customerName}`,
      );
      html = html?.replace(
      ...
      html = html?.replace(
        "{{footer}}",
        `${template?.winningTemplate?.footer}`,
      );
      let mailOptions = {
        from: "Membership App",
        to: data?.customerEmail,
...
export async function sendSMS(to, message) {
  console.log(message, twillioPhone, to, "twillio comsole----------", client);
