    ...
    case "ORDERS_CREATE":
      ...
      try {
        ...
        for (const product of payload?.line_items || []) {
          ...
          const isTicket = product?.properties?.some(
            (p) => p.name === "_ticket" && p.value == "true",
          );

          const name = product?.properties?.find(
            (p) => p.name === "_page_handle",
          )?.value;
          ...
          if (isTicket) {
            ...
            let productGid = `gid://shopify/Product/${product?.product_id}`;
            console.log("productId .......... ", productGid);
                  
            const plan = await planDetailsModel.findOne(
              { name: name },
              { purchasedSpots: 1 }
            );

            const start = plan?.purchasedSpots || 0;


            const result = await planDetailsModel.updateOne(
              {
                name: name,
                $or: [
                  { "products.product_id": productGid },
                  { "subProducts.id": productGid }
                ]
              },
              {
            ...
            const entries = product?.quantity;

            if (Number(entries) <= 0) return;

            const drawIds = Array.from({ length: entries }, (_, i) => {
              return `#${start + i + 1}`;
            });
            ...
            console.log("succesfully===== match value ====");
              ...
              const productGid = `gid://shopify/Product/${product?.product_id}`;
              console.log("productGid ..........", productGid);

              // 🔥 STEP 1: Find + Update using productId (NO name dependency)
              const updatedPlan = await planDetailsModel.findOneAndUpdate(
                {
                  $or: [
                    { "products.product_id": productGid },
                    { "subProducts.id": productGid }
              ...
              if (!updatedPlan) {
                console.log("❌ No plan found using productId");
                return;
              }

              console.log("Updated plan ====>", updatedPlan._id);

              // 🔥 STEP 2: Generate drawIds (SAFE)
              const end = updatedPlan.purchasedSpots;
              const start = end - qty;

              const drawIds = Array.from({ length: qty }, (_, i) => {
                return `#${start + i + 1}`;
              });
              ...
              // 🔥 STEP 3: Optional raffle logic (same as ticket)
              const liveRaffle = await getLiveRaffle(shop);
              let ticketDetails;
          ...
          //   if (result?.matchedCount === 0) {
          //     const res = await planDetailsModel.updateOne(
          //       {
          //         "subProducts.id": productGid,
          //         name: name,
          //       },
