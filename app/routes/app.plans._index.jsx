...
export default function PlanData() {
  ...
  const rows = tableData?.map(({ _id, name, products, plans }, index) => [
    ...
    <InlineStack as="span">
      <span
        onClick={() => {
          (setContentSkel(true), navigate(`/app/plans/${_id}`));
        }}
      >
  ...
  const handleCreate = () => {
    (setContentSkel(true), navigate("/app/plans/create"));
  };
  return (
    <>
      {tableSkel ? (
        ...
        <Page
          title="Raffles"
          primaryAction={{ content: "Create new", onAction: handleCreate }}
        >
          <Card>
            {tableData.length > 0 ? (
              <>
