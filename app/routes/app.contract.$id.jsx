import { Page, DataTable, Modal, Card, Text, Box, Grid, Badge, BlockStack, SkeletonDisplayText, SkeletonBodyText, Button } from "@shopify/polaris"
import { useActionData, useLoaderData, useParams, useSubmit } from "@remix-run/react"
import { json } from "@remix-run/node";
import { authenticate } from "../shopify.server";
import { useEffect, useState } from "react";
import { cancelContract, getConstractDetailById } from "../controllers/planController";
...
export default function ContractDetails() {
    ...
    const rows = tableData?.map((itm, index) => [
        <Text><img src={itm?.variantImage?.url} height={50} width={50} /></Text>,
        <Text> {itm?.title} </Text>,
        <Text> ${itm?.currentPrice.amount} </Text>,
        <Text> {itm?.sellingPlanName?.split('-entries-')?.[1] || 1 * itm?.quantity} </Text>,
        <Text> ${itm?.currentPrice.amount} </Text>,
    ])
                                ...
                                <Card>
                                    ...
                                    <Box paddingBlockStart="200">
                                        <Text as="p" variant="bodyMd">
                                            {capitalizeFirstLetter(data?.customer?.firstName)}{" "}{capitalizeFirstLetter(data?.customer?.lastName)}
                                        </Text>
                                    </Box>
                                </Card>
                            </Grid.Cell>
                    ...
                    <div className="sd-ultimate-option-AlertModal">
                        ...
                        >
                            <Modal.Section>
                                <BlockStack gap={5}>
                                    <p>
                                        Are you sure you want to cancel this contract? This can't
                                        be restored.
                                    </p>
                                </BlockStack>
                            </Modal.Section>
