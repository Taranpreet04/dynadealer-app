    ...
    if (data.length > 0) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (
          ...
          let recentDocument = await billingModel
            .find({
              shop: data[i].shop,
          ...
          if (data[i].billing_policy.interval.toLowerCase() === "day") {
            ...
            data[i].billing_policy.interval.toLowerCase() === "month"
          ) {
            nextDate = new Date(originalDate);
            ...
            await subscriptionContractModel.findOneAndUpdate(
              { shop: data[i].shop, contractId: data[i].contractId },
              {
                $set: {
                  ...
                  ticketDetails: {
                    total: Number(allDrawIds?.length),
                    totalTicketsList: allDrawIds,
                    applied: Number(data[i].ticketDetails?.applied),
                    appliedTicketsList:
                      data[i].ticketDetails?.appliedTicketsList,
                    available:
