document.addEventListener("DOMContentLoaded", () => {
  ...
  function getCurrencySymbol(currencyCode) {
    ...
    return (
      currencySymbols[currencyCode?.toUpperCase() || "USD"] ||
      `Symbol not found for ${currencyCode}`
    );
  }
  ...
  const getContractDetails = async (id, show = false) => {
    try {
      loaderStart();
      const response = await fetch(`${serverPath}/api/getContractDetails`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const cancelStatus = async () => {
    try {
      ...
      const response = await fetch(`${serverPath}/api/cancelContract`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const checkCancelPossible = async () => {
    try {
      const response = await fetch(`${serverPath}/api/checkCancel`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          id: selectedSubscription?.contractId,
          shop: selectedSubscription?.shop,
        }),
      });
      ...
      if (result.message == "success") {
        ...
        if (
          contractDetailDb?.length >=
          selectedSubscription?.billing_policy?.min_cycles
        ) {
    ...
    try {
      let data = {
        ...
        ticketDetails: {
          ...
          applied:
            Number(selectedSubscription?.ticketDetails?.applied) +
          ...
          available:
            Number(selectedSubscription?.ticketDetails?.available) -
            Number(selectedAppliedFor?.applyTicketsCount),
      ...
      const response = await fetch(`${serverPath}/api/appliedTickets`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (result.message == "success") {
        ...
        if (totalList) {
          totalList.innerText =
            result?.details?.ticketDetails?.totalTicketsList?.join(", ");
        }
        let appliedList = document.getElementById("applied-tickets-list");
        if (appliedList) {
          appliedList.innerText =
            result?.details?.ticketDetails?.appliedTicketsList?.join(", ");
        }
        let availableList = document.getElementById("available-tickets-list");
        if (availableList) {
          availableList.innerText =
            result?.details?.ticketDetails?.availableTicketsList?.join(", ");
        }
        ...
        if (tbody) {
          ...
          totalListCell.innerText =
            result?.details?.ticketDetails?.appliedTicketsList
              ?.slice(-Number(selectedAppliedFor?.applyTicketsCount))
              .join(", ") || "jj";
  ...
  const resetModalContent = () => {
    ...
    footer ? footer.appendChild(modal) : "";
  ...
  const generateProductRows = () => {
    ...
    let products = dbData
      ? contractDetailShopify?.products
      : contractDetailShopify?.lines?.edges;
    products?.map((item, index) => {
      ...
      priceCell.innerText = dbData
        ? `${currencySymbol}${item?.price}`
        : `${currencySymbol}${item?.node?.pricingPolicy?.cycleDiscounts[0]?.adjustmentValue?.amount}`;
      let entriesCell = document.createElement("td");
      entriesCell.id = "product-entriesCell";
      entriesCell.innerText = dbData
        ? contractDetailShopify?.ticketDetails?.applied
        : `${selectedSubscription?.ticketDetails?.applied}`;
      let availableEntriesCell = document.createElement("td");
      availableEntriesCell.id = "product-availableEntriesCell";
      availableEntriesCell.innerText = dbData
        ? contractDetailShopify?.ticketDetails?.available
        : `${selectedSubscription?.ticketDetails?.available}`;
      let totalEntriesCell = document.createElement("td");
      totalEntriesCell.id = "product-totalEntriesCell";
      totalEntriesCell.innerText = dbData
        ? contractDetailShopify?.ticketDetails?.total
        : `${selectedSubscription?.ticketDetails?.total}`;
      let totalCell = document.createElement("td");
      ...
      applyBtn.onclick = function () {
        ...
        if (selectedAppliedFor?.applyTicketsCount > 0) {
          ...
          if (selectedAppliedFor?.raffleType === "time-limit") {
            ...
            selectedAppliedFor?.spots ==
              selectedAppliedFor?.applyTicketsCount ||
            selectedAppliedFor?.raffleType !== "capped"
          ) {
            if (modal) {
              modal.innerHTML = modalContentToApplyTickets;
              var span = modal.getElementsByClassName("close")[0];
  ...
  const ticketDetail = () => {
    ...
    appliedListCell.innerText =
      selectedSubscription?.ticketDetails?.appliedTicketsList?.join(", ");
    ...
    restListCell.innerText =
      selectedSubscription?.ticketDetails?.availableTicketsList?.join(", ");
    ...
    totalListCell.innerText =
      selectedSubscription?.ticketDetails?.totalTicketsList?.join(", ");
    tr1.appendChild(totalListCell);
  };
  const showListData = () => {
